...
Orchestrator 应调用 `runCompressionBeforeCompletion({ compression, completion })`。它按目录串行 prepare → acquire → inspect → optional one-shot recovery → authoritative live decision → optional restore → original-source evaluation → release → completion。目录先解析为 filesystem `realpath`，因此 symlink/junction 别名共享 lifecycle lock 和 completion queue；同一 completion async call chain 重入相同 physical directory 会在 lifecycle 前以 non-retryable `LIFECYCLE_LOCKED` fail fast。每次 invocation 在排队前冻结 resolved execution，调用方随后修改 options 不会改变本次决策。Healthy compact live state 未达到 trigger 时不会 restore archive、调用 semantic provider 或重新压缩；达到 trigger 后先恢复 original turns，provider 输入不包含旧 summary。
